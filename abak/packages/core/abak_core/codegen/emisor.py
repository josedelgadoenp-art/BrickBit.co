...
hace Abak, y salen del mismo objeto:
...
def emitir(programa: Programa) -> Emision:
    ...
    for ins in programa.instrucciones:
        ...
        ctx = ContextoEmision(
            ...
            proyeccion=programa.proyeccion, semilla=programa.semilla,
        )
...
def _preludio(programa: Programa, imports: list[Import], ayudantes: list[ast.stmt]) -> ast.Module:
    ...
    if any(obtener(i.op).necesita_datos for i in programa.instrucciones):
        ...
        # el worker pone ABAK_DATOS, y quien descomprime el .zip encuentra
        # `datos/` al lado del script. Sin ramas, sin dos versiones del codigo.
        cuerpo.extend(ast.parse(
            "import os\n"
            "from pathlib import Path\n"
            "RUTA_DATOS = Path(os.environ.get('ABAK_DATOS', 'datos'))\n"
        ).body)
...
def a_texto(emision: Emision, *, con_epilogo: bool = True, autor: str | None = None) -> str:
    """El script `.py` autonomo. Sin `import abak` por ningun lado."""
    p = emision.programa
    ...
    lineas.append("")
    lineas.append("Script generado por Abak. Es el mismo codigo que se ejecuto en el")
    ...
    lineas.append(f"Abak        : v{VERSION_ABAK}")
    ...
    lineas.append("abajo. Abak no es una de ellas.")
    ...
    if con_epilogo and emision.epilogo.body:
        ...
        lineas.extend(_envolver(
            "Al ejecutar dentro de Abak los resultados se leen del espacio de "
