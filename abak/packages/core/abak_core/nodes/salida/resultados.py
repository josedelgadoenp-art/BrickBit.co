...
class Exportar(EspecNodo):
    ...
    ayuda = Ayuda(
        ...
        cuando_usarlo="Al final, cuando el resultado ya se va a usar fuera de Abak.",
    ...
    def emit(self, ctx: Any) -> Any:
        ...
        ctx.nota(f"Se escribe «{nombre}» en la carpeta de salida (variable de entorno ABAK_SALIDA, "
                 "o la carpeta actual).")
        ruta = ctx.temporal("ruta")
        ctx.emitir("R = Path(os.environ.get('ABAK_SALIDA', '.')) / NOMBRE",
