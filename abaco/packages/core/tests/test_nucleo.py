...
RAIZ = Path(__file__).resolve().parents[1] / "abaco_core"
PROHIBIDOS = {"fastapi", "starlette", "celery", "uvicorn", "redis", "abaco_api", "abaco_worker"}


def test_nucleo_puro():
    """`abaco_core` no importa nada de la capa web.
...
def test_se_importa_sin_dependencias_opcionales():
    ...
    assert "abaco_core" in sys.modules or __import__("abaco_core")
