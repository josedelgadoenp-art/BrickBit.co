...
`abaco_core` NO existe. Los coeficientes tienen que salir idénticos.
"""
...
from abaco_core import a_texto, compilar, ejecutar, emitir
...
def test_los_requisitos_no_mencionan_abaco():
    """El script tiene que correr sin instalar Ábaco. Si aparece aquí, no corre."""
    ...
    assert "abaco" not in requisitos.lower().replace("generado por abaco", "")
    assert "import abaco" not in script


@pytest.mark.slow
def test_el_script_exportado_reproduce_el_resultado_sin_abaco():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        entorno = {k: v for k, v in os.environ.items() if k != "ABACO_DATOS"}
        entorno["PYTHONPATH"] = ""      # abaco_core NO está disponible ahí
    ...
    # reproducibilidad.
    from abaco_core.runtime.artefactos import _limpio
    ...
    for variable_, valor in coefs_dentro.items():
        assert valor == _limpio(coefs_fuera[variable_]), (
            f"«{variable_}» difiere entre Ábaco y el script exportado"
        )
