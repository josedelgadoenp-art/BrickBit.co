...
class Ejecutor:
    ...
    def ejecutar(self, programa: Programa, emision: Emision | None = None) -> ResultadoEjecucion:
        ...
        espacio: dict[str, Any] = {"__name__": "__abaco__", "__builtins__": __builtins__}

        try:
            exec(compile(emision.preludio, "<abaco:preludio>", "exec"), espacio)
        ...
        for bloque in emision.bloques:
            ...
            try:
                exec(compile(bloque.arbol, f"<abaco:{ins.nodo_id}>", "exec"), espacio)
            except Exception as exc:
