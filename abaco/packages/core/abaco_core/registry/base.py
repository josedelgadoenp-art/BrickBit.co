...
Agregar una herramienta nueva a Abaco es escribir un archivo de Python. Nada
...
class Ayudante(BaseModel):
    ...
    tentadora es que el script importe `abaco_runtime`, y ahi deja de ser
    ...
    cualquier maquina con pandas y statsmodels, sin Abaco de por medio.
    """
...
def control(tipo: str, **extra: Any) -> dict[str, Any]:
    ...
    El frontend no sabe nada de econometria: lee `abaco.control` y dibuja el
    ...
    """
    return {"abaco": {"control": tipo, **extra}}
...
class EspecNodo:
    """Clase base de toda herramienta de Abaco.
...
def obtener(op: str) -> type[EspecNodo]:
    try:
        ...
        raise ErrorRegistro(
            ...
            f"con una version mas nueva de Abaco."
        ) from None
