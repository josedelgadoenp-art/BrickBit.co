...
**Ábaco** es un sistema de análisis estadístico y econométrico sin código. El usuario
...
Los cuatro sistemas que Ábaco toma como referencia resolvieron cosas distintas:
...
La tesis de Ábaco es que ese eje es falso, y que el punto donde se rompe es este:
...
que corre en cualquier máquina con pandas y statsmodels, sin Ábaco de por medio.
...
`abaco_runtime` — y entonces deja de ser portable, porque exige instalar Ábaco.
...
`pandas`, `statsmodels` y las bibliotecas del análisis en cuestión. Ábaco no aparece
en el archivo.
...
└───────────────┬────────────────────────────────────────────────────┘
                │
┌───────────────▼────────────────────────────────────────────────────┐
│  packages/core — abaco_core   (Python puro, sin dependencias web)  │
│  graph · registry · nodes · codegen · runtime · viz                │
│  Se puede usar como biblioteca, sin API ni frontend.               │
└────────────────────────────────────────────────────────────────────┘
```

`abaco_core` no importa FastAPI ni Celery. Esa regla se prueba (`test_core_puro`):
...
```
 1. Encabezado           → docstring con título, autor, fecha, versión de Ábaco,
...
```python
espacio = {}
exec(compile(preludio, "<ábaco:preludio>", "exec"), espacio)
...
for ins in programa.instrucciones:
    ...
    exec(compile(bloque_de(ins), f"<ábaco:{ins.nodo_id}>", "exec"), espacio)
...
Agregar un nodo nuevo a Ábaco es **escribir un archivo de Python**. Nada más. No se
...
En desarrollo, todo lo anterior cae a disco (`.abaco/`) y Celery corre en modo
...
- **No se reimplementan estimadores.** Ábaco orquesta statsmodels, spreg, scikit-learn
  y XGBoost. Reimplementar econometría es cómo se pierde la confianza que este producto
  necesita. Si un método no está en una biblioteca respetada, no está en Ábaco.
