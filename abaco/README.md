...
Conectas bloques en un lienzo. Ábaco compila ese lienzo a un programa de Python
legible y lo ejecuta. El programa generado no es una bitácora de lo que pasó:
**es** lo que pasó, y lo puedes exportar y correr en cualquier máquina sin Ábaco.
...
> publica en Netlify — `netlify.toml` cierra la ruta `/abaco/*`.
...
de Ábaco es que ese eje es falso: el grafo visual es la interfaz, el programa de
...
```bash
export ABACO_REDIS=redis://localhost:6379/0
...
`abaco_core` no importa FastAPI ni Celery. Es una regla probada
...
descomprime en una carpeta temporal y lo corre en un proceso donde `abaco_core`
...
Es escribir un archivo en `packages/core/abaco_core/nodes/<familia>/`. Nada más:
