...
from abaco_core import GrafoSpec, compilar, emitir
from abaco_core.registry import cargar_todos
from abaco_core.runtime.almacen import ALMACEN
from abaco_core.runtime.cache import CacheDisco
from abaco_core.runtime.ejecutor import Ejecutor
from abaco_core.runtime.exportar import archivos_del_programa

from .celery_app import app

cargar_todos()

LIMITE_MEMORIA_GB = float(os.environ.get("ABACO_LIMITE_MEMORIA_GB", "0") or 0)
...
@app.task(name="abaco.ejecutar_grafo", bind=True)
def ejecutar_grafo(self: Any, ejecucion_id: str, grafo_json: dict[str, Any],
    ...
    # script no sabe nada de como Abaco guarda las cosas.
    ...
    os.environ["ABACO_DATOS"] = str(dir_datos)
    os.environ["ABACO_SALIDA"] = str(ALMACEN.dir_salida(ejecucion_id))
