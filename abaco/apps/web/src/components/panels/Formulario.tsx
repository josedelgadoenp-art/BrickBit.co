 ...
 * los tipos, los rangos y los valores por omisión, y la pista `abaco.control`
...
export default function Formulario({ nodoId, descriptor, params }: Props) {
  ...
  function columnasDe(campo: EsquemaParam, tipoPedido?: string | null): ColumnaEsquema[] {
    const puerto = campo.abaco?.puerto ?? descriptor.entradas[0]?.nombre ?? 'datos';
  ...
  return (
    <div className="space-y-3">
      {Object.entries(propiedades).map(([clave, crudo]) => {
        ...
        if (control === 'columna') {
          const columnas = columnasDe(crudo, crudo.abaco?.tipo_columna);
          control_jsx = (
          ...
          const etiquetas = crudo.abaco?.etiquetas ?? {};
          control_jsx = (
            <select className={clase} value={(valor as string) ?? ''}
